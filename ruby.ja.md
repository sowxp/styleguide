...
- **[SHOULD]** 2つ以上の引数を使う場合は、キーワード引数を必ずつかうこと。
- **[MUST]** 引数を破壊してはいけない
    ```ruby
    # good
    def your_method(str)
...
- **[MUST]** 変数名は英単語を省略せずに使って名付けること。ただしローカル変数では、モデル名などが長くてそのままだと扱いづらい場合は、単語の頭文字をつなげてもよい(例：catalog_order_catalog -> coc)。また、慣習的な名前 (ループカウンタとしての `i` や `j` など) は許可する。
